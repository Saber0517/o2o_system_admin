...
public class UpdataFoodServlet extends HttpServlet {
    ...
    private void UpDateFoodEntity(HttpServletRequest request) throws UnsupportedEncodingException {
        ...
        if (null != foodEntity) {
            resultMap = foodEntityService.updateFoodEntityStatus(foodEntity.getFoodID(),foodEntity.getStatusID());
        } else {
            resultMap = foodEntityService.updateFoodEntity(foodEntity.getFoodID(), foodEntity.getPrice());
        }
    ...
    private FoodEntity getFoodEntity(List<FileItem> items) throws UnsupportedEncodingException {
        ...
        for (FileItem item : items) {
            if (item.isFormField()) {
                ...
                } else if (fileName.equals("stauts")) {
                    foodEntity.setStatusID(Integer.valueOf(item.getString()));
                }
